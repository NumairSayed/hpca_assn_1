...
#include <vector>
#include <iostream>
#include <cstdlib>   // for exit, realloc (C-style) and std::exit
...
// Private helper for RCM: finds a peripheral node in a component
  static
  NodeID_ FindPeripheralNode(const std::vector<std::vector<NodeID_>>& adj,
                             NodeID_ start_node,
                             std::vector<bool>& component_visited) {
    std::vector<int32_t> dist(adj.size(), -1);
    std::queue<NodeID_> q;
    q.push(start_node);
    dist[start_node] = 0;
    if (!component_visited[start_node])
        component_visited[start_node] = true;
    NodeID_ last_node = start_node;
    while (!q.empty()) {
        NodeID_ u = q.front();
        q.pop();
        last_node = u;
  ...
  */
  void MakeCSRInPlace(EdgeList &el, DestID_*** index, DestID_** neighs,
    ...
    *neighs = reinterpret_cast<DestID_*>(el.data());
    ...
    for (int64_t n = num_nodes_; n >= 0; --n)
    ...
    if (!symmetrize_) {   // not going to symmetrize so no need to add edges
      ...
      *neighs = static_cast<DestID_*>(std::realloc(*neighs, newsize));
      ...
      for (int64_t n = (int64_t)num_nodes_ - 1; n >= 0; --n) {
  ...
  */
  ...
  static
    CSRGraph<NodeID_, DestID_, invert> RelabelByRCM(
        const CSRGraph<NodeID_, DestID_, invert> &g) {
      if (g.directed()) {
        std::cout << "Cannot apply RCM on a directed graph" << std::endl;
        std::exit(-12);
      }
      // 1. Build an adjacency list for the RCM algorithm
      std::vector<std::vector<NodeID_>> adj(g.num_nodes());
      #pragma omp parallel for
      for (NodeID_ u = 0; u < g.num_nodes(); u++) {
          for (NodeID_ v : g.out_neigh(u)) {
              adj[u].push_back(v);
      ...
      // 2. Run RCM to get the permutation
      pvector<NodeID_> perm(g.num_nodes());
      std::vector<bool> visited(g.num_nodes(), false);
      pvector<NodeID_> degree(g.num_nodes());
      #pragma omp parallel for
      for(NodeID_ i = 0; i < g.num_nodes(); ++i) degree[i] = adj[i].size();
      
      NodeID_ perm_idx = 0;
      for (NodeID_ i = 0; i < g.num_nodes(); ++i) {
          if (!visited[i]) {
              std::vector<NodeID_> component_perm;
              std::vector<bool> component_visited(g.num_nodes(), false);
              NodeID_ p1 = FindPeripheralNode(adj, i, component_visited);
              std::fill(component_visited.begin(), component_visited.end(), false);
              NodeID_ p2 = FindPeripheralNode(adj, p1, component_visited);
              ...
              while(!q.empty()){
                  NodeID_ u = q.front();
                  q.pop();
                  component_perm.push_back(u);
                  std::vector<NodeID_> neighbors = adj[u];
                  std::sort(neighbors.begin(), neighbors.end(), 
                      [&](NodeID_ a, NodeID_ b) {
                          return degree[a] < degree[b];
                  });
                  for(NodeID_ v : neighbors) {
                      if(!visited[v]) {
                          visited[v] = true;
                          q.push(v);
                      }
              ...
              std::reverse(component_perm.begin(), component_perm.end());
              for(NodeID_ node : component_perm) {
                  perm[perm_idx++] = node;
              }
          }
      }

      // 3. Rebuild the graph with the new permutation
      pvector<NodeID_> new_ids(g.num_nodes());
      #pragma omp parallel for
      for (NodeID_ n=0; n < g.num_nodes(); n++)
        new_ids[perm[n]] = n;

      pvector<NodeID_> new_degrees(g.num_nodes());
      #pragma omp parallel for
      for (NodeID_ n=0; n < g.num_nodes(); n++)
        new_degrees[n] = g.out_degree(perm[n]);
      ...
      #pragma omp parallel for
      for (NodeID_ u_orig=0; u_orig < g.num_nodes(); u_orig++) {
        NodeID_ u_new = new_ids[u_orig];
        for (DestID_ v_orig : g.out_neigh(u_orig))
          neighs[offsets[u_new]++] = new_ids[v_orig];
        std::sort(index[u_new], index[u_new+1]);
      }
