...
class CLConvert : public CLBase {
  ...
  bool out_el_ = false;
  bool out_sg_ = false;
  bool reorder_rcm_ = false;   // NEW FLAG
 ...
 public:
  CLConvert(int argc, char** argv, std::string name)
    ...
    AddHelpLine('r', "", "apply RCM reordering to the graph", "false"); // NEW HELP
  }
  ...
  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      ...
      case 'r': reorder_rcm_ = true;                                    break; // NEW CASE
      default: CLBase::HandleArg(opt, opt_arg);
    }
  }
  ...
  bool reorder() const { return reorder_rcm_; }  // NEW METHOD
};
